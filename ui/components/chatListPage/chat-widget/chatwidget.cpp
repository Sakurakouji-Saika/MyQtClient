    ...
    : QWidget(parent), m_lastViewportWidth(-1)
{
    ...
    m_messagesLayout->setContentsMargins(0, 0, 0, 0); // 关键：去掉底部多余空隙
    ...
    m_updateTimer->setInterval(50); // 50ms 去抖
...
void ChatWidget::addMessage(const QString &text, bool isSelf) {
    ...
    // 仅当是“我”发送的消息时，才自动滚动到底部（并使用 ensureWidgetVisible 提高可靠性）
    if (isSelf) {
        QTimer::singleShot(0, this, [this, b](){
            m_scroll->ensureWidgetVisible(b, 0, 0);
            QScrollBar *bar = m_scroll->verticalScrollBar();
...
void ChatWidget::updateAllBubbleWidths() {
    // 使用 viewport 宽度作为可用宽度来源，并为每条消息减去头像区域宽度
